...
public static class RepeatingButtonEx
{
    ...
    /// <param name="button">
    ///     The button to add the behavior.
    ///     <param>
    ///         <param name="firstDelayMillis">The delay before first repeat in milliseconds.</param>
    ///         <param name="fastestRepeatMillis">
    ///             The smallest delay allowed. Note: Masharling between the timer and the UI
    ///             thread has an unavoidable limit of about 10 milliseconds.
    ///         </param>
    ///         <param name="gradientRate">The new interval is the current interval multiplied by the gradient rate.</param>
    public static void AddRepeatingExponential(this Button button, int firstDelayMillis = 500,
        int fastestRepeatMillis = 15, double gradientRate = 0.75)
    {
        AddRepeating(button, firstDelayMillis, fastestRepeatMillis, gradientRate, null, null, null);
    }

    /// <summary>
    ///     The repeating speed becomes linearily faster.</param>
    ///     <param name="button">
    ///         The button to add the behavior.
    ///         <param>
    ///             <param name="firstDelayMillis">The delay before first repeat in milliseconds.</param>
    ...
    ///                 If specified, the repeats gradually happen more quickly. The new interval is
    ///                 the current interval minus the linear gradient.
    ///             </param>
    public static void AddRepeatingLinear(this Button button, int firstDelayMillis = 500, int fastestRepeatMillis = 50,
        int linearGradient = 25)
    {
        AddRepeating(button, firstDelayMillis, fastestRepeatMillis, null, linearGradient, null, null);
    }

    /// <summary>
    ///     The repeating speed switches from the slow speed to the fastest speed after the specified amount of
    ///     milliseconds.
    /// </summary>
    /// <param name="button">
    ///     The button to add the behavior.
    ...
    ///         <param name="slowToFastestSwitchMillis">
    ///             The delay in milliseconds before switching from the slow repeat speed
    ///             to the fastest repeat speed.
    ///         </param>
    public static void AddRepeatingTwoSpeed(this Button button, int firstDelayMillis = 500,
        int fastestRepeatMillis = 100, int slowRepeatMillis = 300, int slowToFastestSwitchMillis = 2000)
    {
        AddRepeating(button, firstDelayMillis, fastestRepeatMillis, null, null,
            new[] { slowRepeatMillis, fastestRepeatMillis }, new[] { slowToFastestSwitchMillis, 0 });
    }
    ...
    ///         <param name="speedSwitchMillis">
    ///             The delay in milliseconds before switching from one speed state to the next
    ///             speed state.
    ///         </param>
    public static void AddRepeatingThreeSpeed(this Button button, int firstDelayMillis = 500,
        int fastestRepeatMillis = 75, int slowRepeatMillis = 300, int mediumRepeatMillis = 150,
        int speedSwitchMillis = 2000)
    {
        AddRepeating(button, firstDelayMillis, fastestRepeatMillis, null, null,
            new[] { slowRepeatMillis, mediumRepeatMillis, fastestRepeatMillis },
            new[] { speedSwitchMillis, speedSwitchMillis, 0 });
    }

    private static void AddRepeating(this Button button, int firstDelayMillis, int fastestRepeatMillis,
        double? gradientRate, int? linearGradient, int[] speedsMillis, int[] switchesMillis)
    {
        var d = (Data)ht[button];
        if (d != null)
            RemoveRepeating(button);

        d = new Data(button, gradientRate, linearGradient, firstDelayMillis, fastestRepeatMillis, switchesMillis,
            speedsMillis);
        ht[button] = d;
        button.Disposed += delegate { RemoveRepeating(button); };
    }
    ...
    private class Data
    {
        private static readonly MethodInfo methodOnClick;

        public readonly Button Button;
        private int currentSpeed;
        public readonly int FastestRepeatMillis;
        public readonly int FirstDelayMillis;
        public double? GradientRate;

        private DateTime lastEvent = DateTime.MinValue;
        public int? LinearGradient;
        private int millisCount;
        public readonly int[] SpeedsMillis;
        public readonly int[] SwitchesMillis;
        private readonly Timer Timer = new();
        private int waitSum;

        static Data()
        {
            methodOnClick = typeof(Button).GetMethod("OnClick", BindingFlags.Instance | BindingFlags.NonPublic);
        }

        public Data(Button button, double? gradientRate, int? linearGradient, int firstDelayMillis,
            int fastestRepeatMillis, int[] switchesMillis, int[] speedsMillis)
        {
            Button = button;
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    var elapsed = DateTime.UtcNow - lastEvent;
