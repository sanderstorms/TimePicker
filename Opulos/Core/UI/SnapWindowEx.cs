...
public class SnapPoint
{
    ...
    /// <summary>
    ///     For efficiency, a GetWindowRect call is avoided if the childRect is not needed for the Location calculation.
    ///     Subclasses can override this method if needed.
    /// </summary>
    public virtual bool NeedsChildRect => AlignY != 0 || ChildHeightFactor != 0 || AlignX != 0 || ChildWidthFactor != 0;

    public virtual Point GetLocation(RECT childRect, RECT ownerRect)
    {
        var h1 = childRect.Bottom - childRect.Top;
        var h2 = ownerRect.Bottom - ownerRect.Top;
        var availH = Math.Max(0, h2 - h1);
        var y = (int)Math.Round(AlignY * availH + ChildHeightFactor * h1 + ParentHeightFactor * h2 + OffsetConstantY);

        var w1 = childRect.Right - childRect.Left;
        var w2 = ownerRect.Right - ownerRect.Left;
        var availW = Math.Max(0, w2 - w1);
        var x = (int)Math.Round(AlignX * availW + ChildWidthFactor * w1 + ParentWidthFactor * w2 + OffsetConstantX);
...
public static class SnapWindowEx
{
    ...
    private static readonly Hashtable htData = new();

    /// <summary>
    ///     Snaps the child window to the top level owner window, relative to the hWndSnap window. As
    ///     the top level owner window is moved, resized, minimized or maximized, the child window moves too.
    /// </summary>
    ...
    public static void SnapWindow(this Control child, IntPtr hWndParent, IntPtr hWndTopLevel, SnapPoint snapPoint)
    {
        if (child.IsHandleCreated)
            ...
            child.HandleCreated += delegate { SetOwner(child.Handle, hWndParent, hWndTopLevel, snapPoint); };
    }

    public static void SetOwner(IntPtr hWndChild, IntPtr hWndParent, IntPtr hWndTopLevel, SnapPoint snapPoint)
    {
        var d = (Data)htData[hWndChild];
        if (d != null)
        {
            d.nwOwner.ReleaseHandle();
            d.SnapHandle =
                hWndParent; // not sure why, but the hWndParent changes when the Form's font is changed using CTRL+mouse wheel
    ...
    private class Data
    {
        public readonly ChildNW nwChild;
        public OwnerNW nwOwner;
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == WM_WINDOWPOSCHANGED)
            {
                ...
                if (pos.x > -32000 && pos.y > -32000)
                {
                    ...
                    var sp = data.snapPoint;
                    if (sp.NeedsChildRect)
                        GetWindowRect(data.nwChild.Handle, ref rChild);

                    var pt = sp.GetLocation(rChild, rOwner);
                    SetWindowPos(data.nwChild.Handle, IntPtr.Zero, pt.X, pt.Y, 0, 0,
    ...
    private class ChildNW : NativeWindow
    {
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == WM_SHOWWINDOW)
            {
                ...
                var pt = sp.GetLocation(rChild, rOwner);
                SetWindowPos(Handle, IntPtr.Zero, pt.X, pt.Y, 0, 0,
                    SWP_NOOWNERZORDER | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
            }
        }
    }
