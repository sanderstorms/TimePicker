...
public class SpinControl : UserControl, IUpDown
{
    ...
    /// <summary>
    ///     If true, then the width of the buttons and arrows are not dynamically calculated. If false, then
    ///     the images and button widths are calculated when this control receives a Font Changed event.
    /// </summary>
    public bool FixedWidth { get; set; }

    /// <summary>
    ///     If FixedWidth is false, then the width of the up-down arrow images change when the Font Size is changed. The images
    ///     are rendered using a Font Size that is a fraction of the parent Font Size.
    /// </summary>
    public float ArrowFontSizeFactor { get; set; }

    /// <summary>
    ///     For small Fonts, multiplying this control's Font Size by the ArrowFontSizeFactor could produce a font that is too
    ///     small.
    ///     Thus, this value specifies the minimum font size used to render the arrows. The default value is 8f.
    /// </summary>
    public float ArrowMinimumFontSize { get; set; }

    /// <summary>
    ///     If FixedWidth is false, then the width of the up-down buttons change when the Font Size is changed. The widths of
    ///     the buttons
    ///     is determined by the width of the arrow images, multiplied by this factor. The width of the arrow images also
    ///     change when the
    ///     Font is changed, and depend on the ArrowFontSizeFactor. The default value for ButtonWidthFactor is 2f, which
    ///     creates whitespace
    ///     on either side of the arrow. If this value is 1f, then width of the buttons will equal the width of the arrows (but
    ///     only after
    ///     the MinimumSize is reached).
    /// </summary>
    ...
    /// <summary>
    ///     An option to automatically focus on the parent of this control after a button is clicked. The default value is
    ///     true,
    ///     which mimics the native up-down control. However, this means it is not possible to stay focused on the buttons. The
    ///     default
    ///     buttons also have the TabStop property set to false.
    /// </summary>
    public bool FocusParentOnClick { get; set; }

    /// <summary>
    ///     Instead of adding a listener directly to the button, it is better to add the listener to this control (in case
    ...
    private void ResizeButtons()
    {
        ...
        var uh = h2; // up button height
        var dh = h2; // down button height
        var dy = h2;
        ...
        UpButton.Bounds = new Rectangle(0, 0, w, uh);
        DownButton.Bounds = new Rectangle(0, dy, w, dh);
    }
    ...
    protected void UpdateArrowImages()
    {
        ...
        ArrowUpImageDisabled =
            BitmapUtil.CreateIcon(ArrowUpText, ArrowFont, SystemColors.GrayText, antialias: AntiAliasArrows);
        ArrowDownImageDisabled =
            BitmapUtil.CreateIcon(ArrowDownText, ArrowFont, SystemColors.GrayText, antialias: AntiAliasArrows);
    }
    ...
    protected override void OnFontChanged(EventArgs e)
    {
        ...
        {
            var f = ArrowFont;
            ArrowFont = new Font(f.FontFamily, Math.Max(ArrowMinimumFontSize, ArrowFontSizeFactor * Font.Size), f.Style,
                f.Unit, f.GdiCharSet, f.GdiVerticalFont);
            UpdateArrowImages();
            var w2 = (int)Math.Round(ButtonWidthFactor * Math.Max(ArrowUpImage.Width, ArrowDownImage.Width));
    ...
    {
        var sz = btn.Size;
        var s = btn.FlatStyle == FlatStyle.Flat ? ButtonState.Flat : ButtonState.Normal;
        if (!btn.Enabled)
        {
    ...
    {
        var p = upButton.Padding;
        var s = upButton.Size;
        var aw = s.Width - p.Horizontal;
        var ah = s.Height - p.Vertical;
        var ih = upButtonImage.Size;
        var x = p.Left + (aw - ih.Width) / 2;
        var dy = ah - ih.Height;
        var dy2 = dy / 2;
        var y = p.Top + dy2;
    ...
    {
        var p = downButton.Padding;
        var s = downButton.Size;
        var aw = s.Width - p.Horizontal;
        var ah = s.Height - p.Vertical;
        var ih = downButtonImage.Size;
        var x = p.Left + (aw - ih.Width) / 2;
        var dy = ah - ih.Height;
        var y = p.Top + dy / 2 + (dy % 2 == 1 ? 1 : 0);
    ...
    private static void DrawModernArrow(Graphics g, Rectangle r, ScrollBarArrowButtonState state)
    {
        if (r.Width < 17 || r.Height < 18)
            ...
            // is used.
            using (var bmp = new Bitmap(Math.Max(20, r.Width), Math.Max(20, r.Height)))
            {
                using (var g2 = Graphics.FromImage(bmp))
                {
                    ScrollBarRenderer.DrawArrowButton(g2, new Rectangle(0, 0, bmp.Width, bmp.Height), state);
                    using (var bmp2 = bmp.Clone(
                               new Rectangle((bmp.Width - r.Width) / 2, (bmp.Height - r.Height) / 2, r.Width, r.Height),
                               bmp.PixelFormat))
                    {
    ...
    public class SpinButton : Button
    {
        ...
        private bool _showFocusCue;
        private readonly SpinControl Owner;
        ...
        protected override bool ShowFocusCues => _showFocusCue;

        protected override void OnEnabledChanged(EventArgs e)
        {
            base.OnEnabledChanged(e);
            BackColor = Enabled ? SystemColors.ControlLight : SystemColors.Control;
            ForeColor = Enabled ? SystemColors.AppWorkspace : SystemColors.ControlLight;
