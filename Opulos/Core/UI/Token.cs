...
public class Token
{
    internal Token(int seqNo, string text, string mask)
    {
        ...
        MaxValue = max;
        BigIncrement = Math.Max(2, max / 20);
        CarryOverScope = int.MaxValue;
    }
    ...
    /// <summary>
    ///     A continuous sequence of characters that are all editiable or all literals. The text is a substring of the
    ///     MaskedTextBox's Text property (including prompts and literals) at the time when the Mask was changed.
    /// </summary>
    public string Text { get; internal set; }

    /// <summary>
    ///     The index position of the first character of the raw text in the overall Text value at the time the Mask was
    ///     changed.
    /// </summary>
    ...
    /// <summary>
    ///     True means that the characters in RawText are literals, as determined by the MaskedTextBox's
    ///     MaskedTextProvider at the time the Mask was changed.
    /// </summary>
    public bool IsLiteral { get; internal set; }

    /// <summary>
    ///     The minimum value this token can take when the user changes the value by typing a key, the mouse wheel or spinner
    ///     buttons.
    ...
    /// <summary>
    ///     The maximum value this token can take when the user changes the value by typing a key, the mouse wheel or spinner
    ///     buttons.
    ///     The default value is the power of 10 to the length of the RawText.
    /// </summary>
    ...
    /// <summary>
    ///     Specifies if the token to the left increments or decrements if this token's value is wrapped. If this value is
    ///     null,
    ///     then the MaskedTextBox ValuesCarryOver property is used. Note: CarryOver is ignored if ByDigit is true.
    /// </summary>
    ...
    /// <summary>
    ///     There are two different behaviors for changing a token's value. ByDigit true means that the digit
    ...
    /// <summary>
    ///     Returns the unescaped version of the mask, i.e. the backslashes are removed from the literals
    ///     and the &gt; &lt; and | special characters are skipped.
    /// </summary>
    ...
    /// <summary>
    ///     The length of the corresponding characters in the mask, which could be longer than Length. For example, if the Mask
    ///     contains
    ///     special characters that change to uppercase or lowercase.
    /// </summary>
    public int MaskRawLength => MaskRaw.Length;

    /// <summary>
    ///     Returns an array that is the same length as the Mask property, where a true value means the
    ///     character is converted to uppercase, false to lowercase, and null means no conversion.
    /// </summary>
    public bool?[] Shifts { get; internal set; }

    /// <summary>
    ///     A custom list of characters that the MaskedTextBox will cycle through. If null, then the
    ///     a default list is used based on the allowable characters for the mask position, and whether a shift
    ///     control character is used to convert to uppercase or lowercase characters.
    /// </summary>
    public IList<char> ByDigitChars { get; set; }

    /// <summary>
    ///     If specified, then the custom value is padded with this character. The property applies for
    ...
    /// <summary>
    ///     The default value is to pad the characters on the right side. This this property to true to
    ...
    /// <summary>
    ///     An option to switch the up arrow or page up with the down arrow or page down. For example, a
    ...
    /// <summary>
    ///     An option to override the default value set in the MaskedTextBox parent. Only applies when
    ///     KeepTokenSelected is true.
    /// </summary>
    public bool? KeepSelectedIncludesWhitespace { get; set; }

    /// <summary>
    ///     Returns a bitwise enum value that indicates which characters can be typed in this token.
    ///     Multiple flags might be set if a token mixes multiple different wildcard characters in a row.
    /// </summary>
    public TokenInputFlags AllowedChars
    {
        get
        {
            ...
            foreach (var c in Mask)
                ...
                else if (c == '?' || c == 'C') f = f | TokenInputFlags.Letters | TokenInputFlags.Spaces;
    ...
    /// <summary>
    ///     A custom list of values can be assigned to a token, which defines the allowable values that
    ///     can be selected. The selectable values from one token, may depend on the current value of another token,
    ///     so it is possible to listen for the TokenChanging event and assign the CustomValues dynamically.
    /// </summary>
    public IList<object> CustomValues { get; set; }

    /// <summary>
    ///     The MaskedTextProvider for the entire MaskedTextBox as provided by the MaskedTextBox after the Mask property
    ///     is set.
    /// </summary>
    public MaskedTextProvider OwnerTextMaskProvider { get; internal set; }

    /// <summary>
    ///     Returns true if this token has non-empty display text and is either not a literal or has a non-null
    ///     CustomValues list.
    /// </summary>
    public bool CanEdit =>
        // Text.Length == 0 if the token only contain shifting characters
        Text.Length > 0 && (!IsLiteral || CustomValues != null);
