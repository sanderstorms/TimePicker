...
public class ToolStripDropDownAttacher : IMessageFilter
{
    ...
    private readonly NW2 nw2 = new();
    private readonly NW3 nw3;
    ...
    public ToolStripDropDownAttacher(ToolStripDropDown menu, Control control, bool keepMenuOpen = true)
    {
        ...
        AutoCloseReasonWindowLostFocus = keepMenuOpen ? null : ToolStripDropDownCloseReason.AppFocusChange;
...
//			}
            ...
            if (isClosing || e.CloseReason == ToolStripDropDownCloseReason.ItemClicked ||
        ...
        menu.Click += delegate
        {
            isFocusing = true;
            menu.Focus();
            var h = GetTopWindow(control.Handle);
        ...
        menu.GotFocus += delegate
        {
            ...
            var activate = true;
            var h1 = GetTopWindow(control.Handle);
            var h3 = GetWindow(h1, GW_OWNER);
            if (h3 != IntPtr.Zero)
            {
...
//Debug.WriteLine("Skipping lost focus");
            ...
            {
                Debug.WriteLine("Menu lost focus and window is not top window.");
...
Debug.WriteLine("focus lost, closing menu");
        ...
        if (control.IsHandleCreated)
            ...
            control.HandleCreated += delegate { nw2.AssignHandle(control.Handle); };
            ...
            combo.DropDown += delegate
            {
                if (MirrorControlDropDown)
                {
                    ...
                    if (!Menu.Visible)
                        ...
                        {
                            Debug.WriteLine(
                                "combo.DropDownOpened: BeginInvoke > ShowMenu(), preventClose=true, Menu.Focus(), SelectFirstItem(), preventClose=false");
        ...
        menu.VisibleChanged += delegate
        {
            if (!menu.Visible && menu.AutoClose)
            {
                ...
                Control.BeginInvoke((Action)delegate { ignore = false; });
            }
        };

        //bool isMouseDown = false;
        var wasJustOpened = false;
        MouseEventHandler mouseDown = (o, e) =>
        {
            if (ignore)
                return;
        ...
        MouseEventHandler mouseUp = (o, e) =>
        {
            // toggle the menu visibility
            //bool none = (Control.MouseButtons == MouseButtons.None);
            //!isMouseDown
            if (!wasJustOpened && ClickOpensMenu && Menu.Visible && ClickCloseReason.HasValue &&
                (!CanMirror || !MirrorControlDropDown))
        ...
        var reallyLostFocus = true; // added 2016-01-25
        //control.mouse
        control.GotFocus += delegate
        {
            ...
            var mouseActivate = ClickOpensMenu && nw2.MouseActivate;
            ...
            // was activated.
            if (!menu.Visible && !isClosing && reallyLostFocus && GotFocusOpensMenu && !mouseActivate &&
                (!CanMirror || !MirrorControlDropDown)) ShowMenu();
        ...
        control.LostFocus += delegate
        {
            ...
            var h1 = GetForegroundWindow();
            var isTaskbar = IsTaskbar(h1);
            if (isTaskbar)
                return;

            var f = control.FindForm(); // f might be null if mixing WPF (todo: support WPF)
        ...
        control.KeyDown += (o, e) =>
        {
            if (e.KeyData == Keys.Escape && (!CanMirror || !MirrorControlDropDown))
            {
                ...
                if (menu.Visible && EscapeCloseReason.HasValue && !menu.AutoClose) CloseMenu(EscapeCloseReason.Value);
            }
        ...
        control.EnabledChanged += delegate { menu.Enabled = control.Enabled; };

        control.FontChanged += delegate
        {
            var f = control.Font;
            var f2 = menu.Font;
            var ff = f2.FontFamily;
            ...
            var newSize = Math.Max(MinFontSize, f.Size + FontSizeDelta);
            var f3 = new Font(ff, newSize, f2.Style, f2.Unit, f2.GdiCharSet, f2.GdiVerticalFont);
            menu.Font = f3;
    ...
    public ToolStripDropDown Menu { get; }

    ///<summary>The control passed to the constructor.</summary>
    public Control Control { get; }

    /// <summary>
    ///     Set to a non-null value to auto-close the menu if the focus is lost to another control in the same top level
    ///     window.
    /// </summary>
    ...
    /// <summary>
    ///     An option to re-show the menu when the top level window becomes the foreground window, and the control has the
    ///     focus.
    /// </summary>
    public bool ShowMenuOnActivate { get; set; }

    /// <summary>
    ///     The minimum font size allowed for the drop down menu. The default value is 6. The menu automatically changes
    ///     font size when the control's font size is changed.
    /// </summary>
    public float MinFontSize { get; set; }

    /// <summary>
    ///     When the control's font size is changed, the menu's font size is the control's font size plus the delta value.
    ///     If all the items in the menu use the same font size as the menu's font size, then a delta value zero will probably
    ///     look
    ///     okay. However, if the menu has items that scale up with its base font size, then using a negative delta might look
    ///     better.
    ...
    /// <summary>
    ///     A list of keys that when pressed will open the menu when the control has the focus. The default list only
    ///     contains Keys.Down.
    /// </summary>
    public List<Keys> OpenMenuKeys { get; }

    /// <summary>
    ///     An option to have a mouse click on the control open the menu. Some controls may not want to open the menu on a
    ///     click. The default value is true.
    /// </summary>
    public bool ClickOpensMenu { get; set; }

    /// <summary>
    ///     Set to a non-null value to close the menu if it is currently visible and the control is clicked.
    ///     ClickOpensMenu must be true for this to work. The default value is CloseCalled.
    /// </summary>
    ...
    /// <summary>
    ///     If non-null, this is the value passed to the CloseMenu(...) method when the escape key is pressed. If null then
    ///     Escape does not close the menu.
    ///     The default value is ToolStripDropDownCloseReason.Keyboard.
    /// </summary>
    public ToolStripDropDownCloseReason? EscapeCloseReason { get; set; }

    /// <summary>
    ///     Defines if the menu aligns to the left of the control or to the right of the control. The default is left
    ///     alignment.
    /// </summary>
    ...
    private bool CanMirror => nw3 != null;
...
//Debug.WriteLine("attacher:" + DateTime.Now.ToString("HH:mm:ss:") + m);
        ...
        if (m.Msg == WM_KEYDOWN)
        {
            ...
            if (vk == VK_TAB)
            {
                ...
                {
                    var b = m.HWnd == Control.Handle;
                    if (!b)
                    {
                        var c2 = Control.FromChildHandle(m.HWnd);
    ...
    private static void GetAllControls(Control host, List<Control> list, bool nested, int currentCount, int maxItems)
    {
        ...
        foreach (var c in host.Controls.Cast<Control>().OrderBy(c => c.TabIndex))
            if (c.CanSelect)
            {
                list.Add(c);
    ...
    private static void GetAllItems(List<object> list, ToolStripItemCollection items, bool nested,
        ref int selectedIndex, int maxItems = int.MaxValue)
    {
        for (var i = 0; i < items.Count; i++)
        {
            var item = items[i];
            if (item.IsOnOverflow) continue; // is this correct?
    ...
    {
        var x = -1;
        var list = GetAllItems(Menu.Items, nested, out x);
        if (list.Count == 0)
            return null;

        for (var i = 0; i < list.Count; i++) Debug.WriteLine(i + " " + list[i]);

        Debug.WriteLine("selected item: " + x);
    ...
    {
        var isTaskbar = hwnd == FindWindow("Shell_TrayWnd", "");
        return isTaskbar;
    }
    ...
    {
        var h1 = GetTopWindow(attacher.Control.Handle);
        var h2 = GetForegroundWindow();
        return h1 == h2;
    }
    ...
    public void CloseMenu(ToolStripDropDownCloseReason reason)
    {
        ...
        if (CanMirror && MirrorControlDropDown)
        {
            if (Control is ComboBox)
            {
                ...
                {
                    Debug.WriteLine("setting dorpped down flase");
                    combo.DroppedDown = false;
                }
                else
                {
                    Debug.WriteLine("CloseMenu(" + reason + ")");
                    isClosing = true;
                    Menu.Close(reason);
                    isClosing = false;
                }
    ...
    public bool ShowMenu()
    {
        ...
        var hWnd = GetTopWindow(Control.Handle);
        var factor1 = Alignment == HorizontalAlignment.Left ? 0 : -1;
        var factor2 = Alignment == HorizontalAlignment.Left ? 0 : 1;
        Menu.SnapWindow(Control.Handle, hWnd,
            new SnapPoint
            {
                ParentHeightFactor = 1, OffsetConstantY = 1, ChildWidthFactor = factor1, ParentWidthFactor = factor2
            });
        PositionWindow();
        return true;
    }
    ...
    private void PositionWindow()
    {
        ...
        var pt = Alignment == HorizontalAlignment.Left
            ? new Point(r.Left, r.Bottom + 1)
            : new Point(r.Right - Menu.Width, r.Bottom + 1);
        Menu.Show(pt);
    ...
    private class NW : NativeWindow
    {
        ...
        public static readonly IList<int> ignoreList = new[]
        {
            0x133, 0xd, 0xe, 0xf, 0x282, 0x281, 0x46, 0x47, 0x14, 0x85, 0x111, 0xc1e8, 0x2e0, 0x55, 0x129, 0x210, 0x200,
            0x2a3, 0x118,
            0xc0ab, 0x113, 0xa0, 0x2a2, 0x2a1, 0x31f, 0x5, 0x83, 0x84, 0x400, 0x20, 0x820712, 0xc1a9, 0x87, 0xc260,
            0xd00c6, 0xc1e4, 0x3, 0x20a, 0x20e
        };
...
//System.Diagnostics.Debug.WriteLine("WP: " + wp.showCmd);
            ...
            {
                Debug.WriteLine("WM_ACTIVATEAPP WParam: " + m.WParam + " Menu.Visible: " + attacher.Menu.Visible +
                                " IsAdjusting: " + attacher.isAdjusting + " WasMinimized: " + attacher.wasMinimized);

                if (m.WParam.ToInt32() == 0)
                {
                    Debug.WriteLine("WM_ACTIVATEAPP: window deactivated, hiding menu");
                    HideMenu(attacher);
                }
        ...
        private static void HideMenu(ToolStripDropDownAttacher attacher)
        {
            if (!attacher.isAdjusting && attacher.Menu.Visible)
            {
                ...
                var h1 = GetForegroundWindow();
                var h2 = GetTopWindow(attacher.Control.Handle);
                var b1 = h1 == attacher.Menu.Handle;
                var b2 = h1 == attacher.Control.Handle; // probably never happens
                var b3 = h1 == h2;
                if (!b1 && !b2 && !b3) hideMenu = true;
                ...
                if (hideMenu)
                {
                    ...
                    else
                    {
                        ...
                        SetWindowPos(attacher.Menu.Handle, (IntPtr)1, 0, 0, 0, 0,
